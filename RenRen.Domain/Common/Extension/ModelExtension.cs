    ...
    public static class ModelExtension
    {
        ...
        public static void CopyTo<Source, Tag>(this Source sourct, Tag tag, bool ignoreId = true, bool ignoreEmpty = false)
        {
            ...
            foreach (PropertyInfo source in Types.GetProperties())
            {
                ...
                {
                    if (!(ignoreId && tagProp.Name.Equals("id", StringComparison.OrdinalIgnoreCase)) && tagProp.Name.Equals(source.Name, StringComparison.OrdinalIgnoreCase))
                    {
                        if (source.GetValue(sourct) != null)
                        {
